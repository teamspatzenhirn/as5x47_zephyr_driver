# Example for AS5X47 Zephyr Driver Module
## Prerequisites
This example is build for Teensy 4.1. It is expected that this module is cloned in a directory called `as5x47` inside a `modules` directory together with the [LPSPI driver](https://github.com/teamspatzenhirn/lpspi_zephyr_driver) used. This can happen either manually, via git submodules or using west. Moreover, zephyr (tested with v4.3.0) must be installed and CMake must be able to find it.
